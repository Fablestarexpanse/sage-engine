...
- [ ] `python -m pytest -q` (no env var) passes and reports the live tests as skipped.
- [ ] `SAGE_LIVE_TESTS=1 python -m pytest -q -m live` passes locally against Docker services
      with 5 tests passed.
- [ ] After the live run, `psql`/asyncpg shows no remaining `sage_live_*` database.
- [ ] The dev database's `alembic_version` and row counts are unchanged by the live run.
- [ ] `.github/workflows/ci.yml` has jobs `python`, `worldforge`, `live`.
- [ ] The four gates pass.
