//! Every 10 ticks, moves each located entity along the first link out of where it is. Uses both
//! `entities` and `space`.
...
impl Guest for Plugin {
    ...
    fn run(tick: u64) -> Result<Vec<EventRecord>, String> {
        ...
        for id in entities::entities_with("sage.located") {
            let located = entities::get_component(id, "sage.located").ok_or("entity vanished")?;
            let located: serde_json::Value =
                serde_json::from_str(&located).map_err(|e| e.to_string())?;
            let here = located["within"].as_u64().ok_or("within is not an id")?;
