...
import { Icons, Badge, StatusDot, ActionButton, SearchBar, DataTable, FetchErrorBanner } from "../adminCommon.jsx";

// Live › Who's online: connected play sessions and agents (marked), refreshed every few seconds.
...
const OnlinePage = () => {
  ...
  useEffect(() => {
    let alive = true;
    const load = () => axios.get(`${API_BASE}/players`, { params: { include_agents: showAgents } })
  ...
  const agents = (sessions ?? []).filter((p) => p.agent).length;
  const players = (sessions ?? []).length - agents;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 12, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: COLORS.textMuted, fontFamily: "'DM Sans', sans-serif" }}>
            {sessions == null ? "Loading…" : `${players} player${players === 1 ? "" : "s"}${showAgents ? `, ${agents} agent${agents === 1 ? "" : "s"}` : ""}`}. Refreshes every 5 seconds.
          </p>
        </div>
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: COLORS.textMuted, cursor: "pointer" }}>
            <input id="online-show-agents" type="checkbox" checked={showAgents} onChange={(e) => setShowAgents(e.target.checked)} />
            Show agents
          </label>
      ...
      <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 10, overflow: "auto" }}>
        <DataTable columns={[
          ...
          { label: "Kind", render: (row) => (row.agent ? <Badge color={COLORS.info}>agent</Badge> : <Badge color={COLORS.success}>player</Badge>) },
          ...
          { label: "", render: (row) => (
            <div style={{ display: "flex", gap: 4 }}>
              {row.agent && (
                <ActionButton small variant="ghost" title="Open agents" onClick={() => { window.location.hash = "/agents"; }}><Icons.Eye /></ActionButton>
              )}
              {row.accountId != null && !row.agent && (
